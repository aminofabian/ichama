...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeGuarantees.length > 0) {
      // Get details about borrowers who haven't completed payments
      const guaranteesWithDetails = await Promise.all(
        activeGuarantees.map(async (guarantee) => {
          ...
          if (remainingAmount <= 0) return null // Skip fully paid loans
      ...
      const unpaidGuarantees = guaranteesWithDetails.filter(
        (g): g is NonNullable<typeof g> => g !== null
      )

      if (unpaidGuarantees.length > 0) {
        const borrowerNames = unpaidGuarantees.map((g) => g.borrowerName).join(', ')
        return NextResponse.json<ApiResponse>(
          {
            success: false,
            error: `You cannot request a loan because ${borrowerNames} ${unpaidGuarantees.length > 1 ? 'have' : 'has'} yet to complete their loan payment${unpaidGuarantees.length > 1 ? 's' : ''}. Please wait until the loans you guaranteed are fully paid.`,
          },
          { status: 400 }
        )
      }
