...
async function fixSavingsBalances() {
  ...
  // Get all confirmed contributions with their cycle info
  const contributions = await db.execute({
    sql: `
      ...
      WHERE c.status IN ('paid', 'confirmed')
    `,
    args: [],
  })
  ...
  for (const row of contributions.rows) {
    ...
    // Savings = amount paid beyond contribution, capped at savings target
    const savingsFromPayment = Math.max(0, c.amount_paid - c.contribution_amount)
    const actualSavings = Math.min(savingsTarget, savingsFromPayment)
    ...
    console.log(`Contribution ${c.id.slice(0,8)}...: paid ${c.amount_paid}, contrib ${c.contribution_amount}, savings portion: ${actualSavings}`)
  }
